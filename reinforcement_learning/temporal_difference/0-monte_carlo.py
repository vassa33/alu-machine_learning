...
def generate_episode(env, policy, max_steps):
    """
        Generates an episode of frozen lake.
    """
    episode = [[], []]
    state = env.reset()
...
def monte_carlo(env, V, policy, episodes=5000, max_steps=100, alpha=0.1, gamma=0.99):
    ...
    for ep in range(episodes):
        ...
        for i in range(len(episode[0])):
            Gt = np.sum(np.array(episode[1][i:]) * np.array(discounts[:len(episode[1][i:])]))
            V[episode[0][i]] = V[episode[0][i]] + alpha * (Gt - V[episode[0][i]])
